...
url = "/cgi-bin/perseus/citequery3.pl?dbname=GreekDec20&getid=0&query=Pl.%20Cri.%2043a"
r = requests.get("http://artflsrv02.uchicago.edu/" + url)
outer_soup = BeautifulSoup(r.text, "html.parser")

words = defaultdict(list)
section = None
next_url = outer_soup.find("td", align="right").contents[0]["href"]
while next_url != url:
    ...
    while (True in ("id" in parent.attrs and parent["id"] == "perseuscontent" for parent in tag.parents)):
        if tag.name == "w":
            ...
            print("STEPPING DOWN")
            print(tag)
            if (tag.name == 'p') or ("class" in tag.attrs and "bibl" not in tag["class"]):
                tag = tag.contents[0]
                continue
...
with open('lemmatized_crito.json', 'w') as fp:
    json.dump(words, fp)