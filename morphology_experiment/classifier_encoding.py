...
train, val, test = tt.data.TabularDataset.splits(
        path='../data/',
        format='csv',
        train="greek_corpus_train_encoded_labeled.csv",
        validation="greek_corpus_valid_encoded_labeled.csv", 
        test="greek_corpus_test_encoded_labeled.csv",
        fields=[('text', TEXT), ('label', LABEL)],
        csv_reader_params={'delimiter':'\t'},
        )

apology = tt.data.TabularDataset(
    path='../data/greek_corpus_apology_encoded_labeled.csv',
    format="csv",
    csv_reader_params={'delimiter':'\t'},
    fields=[('text', TEXT), ('label', LABEL)]
)
...
apology_accs = []
for i in range(10):
    classifier = LSTMClassifier(TEXT, LABEL, embedding_size=64, hidden_size=32, experiment=f"runs/byte_pair_fixed_{i}").to(device)
...
with open('./results.csv', 'a') as results_csv:
    ...
    writer.writerow(["byte_pair_fixed", train_accs.mean(), train_accs.std(), test_accs.mean(), test_accs.std(), apology_accs.mean(), apology_accs.std()])
